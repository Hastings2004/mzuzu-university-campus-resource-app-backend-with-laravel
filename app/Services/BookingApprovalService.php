...
class BookingApprovalService
{
    ...
    /**
     * Mark a booking as in use (change status from approved to in use).
     *
     * @param int $bookingId
     * @param int $adminId
     * @param string|null $notes
     ...
     * @throws BookingApprovalException
     */
    public function markBookingAsInUse(int $bookingId, int $adminId, ?string $notes = null): Booking
    {
        DB::beginTransaction();
        try {
            $booking = Booking::findOrFail($bookingId);
            ...
            if ($booking->status !== Booking::STATUS_APPROVED) {
                throw new BookingApprovalException('Only approved bookings can be marked as in use.', 400);
            }

            // Check if the booking time has started
            if ($booking->start_time->gt(Carbon::now())) {
                throw new BookingApprovalException('Cannot mark booking as in use before the scheduled start time.', 400);
            }

            // Check if the booking time has already ended
            if ($booking->end_time->lt(Carbon::now())) {
                throw new BookingApprovalException('Cannot mark booking as in use after the scheduled end time.', 400);
            }

            $booking->update([
                'status' => Booking::STATUS_IN_USE,
                'in_use_started_by' => $adminId,
                'in_use_started_at' => Carbon::now(),
                'admin_notes' => $notes,
            ]);

            Log::info("Booking {$bookingId} marked as in use by admin {$adminId}");
            DB::commit();
            ...
            DB::rollBack();
            Log::error('Error marking booking as in use: ' . $e->getMessage(), ['booking_id' => $bookingId, 'admin_id' => $adminId, 'trace' => $e->getTraceAsString()]);
            throw new BookingApprovalException('Failed to mark booking as in use.');
        }
    }

    /**
     * Mark a booking as completed (change status from in use to completed).
     *
     * @param int $bookingId
     * @param int $adminId
     * @param string|null $notes
     ...
     * @throws BookingApprovalException
     */
    public function markBookingAsCompleted(int $bookingId, int $adminId, ?string $notes = null): Booking
    {
        DB::beginTransaction();
        try {
            $booking = Booking::findOrFail($bookingId);
            ...
            if ($booking->status !== Booking::STATUS_IN_USE) {
                throw new BookingApprovalException('Only in use bookings can be marked as completed.', 400);
            }
            ...
            Log::info("Booking {$bookingId} marked as completed by admin {$adminId}");
            DB::commit();
            ...
            DB::rollBack();
            Log::error('Error marking booking as completed: ' . $e->getMessage(), ['booking_id' => $bookingId, 'admin_id' => $adminId, 'trace' => $e->getTraceAsString()]);
            throw new BookingApprovalException('Failed to mark booking as completed.');
        }
    }

    public function inUseApproval(int $bookingId, int $adminId, ?string $notes = null): Booking
    {
        // This method seems to have incorrect logic - it should be removed or renamed
        // For now, I'll redirect it to the new method
        return $this->markBookingAsInUse($bookingId, $adminId, $notes);
    }
}
